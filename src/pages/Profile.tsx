...
const Profile = () => {
  ...
  const fetchLabData = async (userId: string) => {
    const { data, count } = await supabase
      ...
      .limit(1);
    if (data && data.length > 0) setLatestLab(data[0]);
  ...
  const getThyroidBadge = () => {
    if (!latestLab) return { label: "No Data", color: "bg-white/10 text-white/60" };
    const tsh = latestLab.tsh_level;
      ...
      <div className="relative z-10 px-6 pb-20 max-w-4xl mx-auto space-y-8">
              ...
              <div className="flex-1 text-center md:text-left space-y-2">
                {isEditingProfile ? (
                  ...
                  <>
                    ...
                    <p className="text-white/60 flex items-center gap-2 justify-center md:justify-start"><Calendar className="w-4 h-4" />Last report: {latestLab ? new Date(latestLab.created_at).toLocaleDateString() : "No reports yet"}</p>
          ...
          <CardContent className="p-8">
            ...
            {latestLab ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { label: "TSH", value: latestLab.tsh_level, unit: "µIU/mL", status: latestLab.tsh_status },
                  { label: "T3", value: latestLab.t3_level, unit: "ng/dL", status: latestLab.t3_status },
                  { label: "T4", value: latestLab.t4_level, unit: "µg/dL", status: latestLab.t4_status },
                ].map((item) => (
                  <div key={item.label} className="bg-white/5 border border-pink-400/20 rounded-xl p-4 text-center">
                    ...
                    <Badge className={`mt-2 ${item.status === "normal" ? "bg-green-500/20 text-green-300" : item.status === "elevated" ? "bg-yellow-500/20 text-yellow-300" : item.status === "low" ? "bg-orange-500/20 text-orange-300" : "bg-white/10 text-white/60"}`}>
                      {item.status || "unknown"}
                    </Badge>
                  </div>
                ))}
        ...
        <Card className={cardClass}>
          <CardContent className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { label: "Total Reports", value: totalReports, color: "text-pink-300" },
                { label: "Last Uploaded", value: latestLab ? new Date(latestLab.created_at).toLocaleDateString() : "N/A", color: "text-purple-300" },
                { label: "Lab Analyses Done", value: totalReports, color: "text-blue-300" },
              ].map((item) => (
                <div key={item.label} className="bg-white/5 border border-pink-400/20 rounded-xl p-4 text-center">
                  <p className="text-white/60 text-sm mb-1">{item.label}</p>
                  <p className={`text-2xl font-bold ${item.color}`}>{item.value}</p>
          ...
          <CardContent className="p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                ...
                <Select value={healthDetails.diagnosisType} onValueChange={(v) => setHealthDetails({ ...healthDetails, diagnosisType: v })} disabled={!isEditingHealth}>
                  ...
                  <SelectContent className="bg-[#2a1050] border-white/20">
                    ...
                    <SelectItem value="subclinical-hypo">Subclinical Hypothyroidism</SelectItem>
                    <SelectItem value="subclinical-hyper">Subclinical Hyperthyroidism</SelectItem>
              ...
              <div className="space-y-2">
                ...
                <Input value={healthDetails.doctorName} onChange={(e) => setHealthDetails({ ...healthDetails, doctorName: e.target.value })} disabled={!isEditingHealth} className="bg-white/10 border-white/20 text-white disabled:opacity-60" placeholder="Dr. Name" />
              </div>
              <div className="space-y-2">
                <Label className="text-white/70">Emergency Contact Name</Label>
                <Input value={healthDetails.emergencyContactName} onChange={(e) => setHealthDetails({ ...healthDetails, emergencyContactName: e.target.value })} disabled={!isEditingHealth} className="bg-white/10 border-white/20 text-white disabled:opacity-60" placeholder="Contact name" />
              </div>
              <div className="space-y-2">
                <Label className="text-white/70">Emergency Contact Phone</Label>
                <Input value={healthDetails.emergencyContactPhone} onChange={(e) => setHealthDetails({ ...healthDetails, emergencyContactPhone: e.target.value })} disabled={!isEditingHealth} className="bg-white/10 border-white/20 text-white disabled:opacity-60" placeholder="Phone number" />
              </div>
              <div className="space-y-2 md:col-span-2">
                <Label className="text-white/70">Health Goals</Label>
                <Input value={healthDetails.healthGoals} onChange={(e) => setHealthDetails({ ...healthDetails, healthGoals: e.target.value })} disabled={!isEditingHealth} className="bg-white/10 border-white/20 text-white disabled:opacity-60" placeholder="e.g. Maintain TSH in range, exercise daily" />
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="space-y-2">
              <button onClick={() => setIsEditingProfile(true)} className="w-full bg-white/5 border border-pink-400/20 rounded-xl p-4 flex items-center justify-between hover:bg-white/10 transition-all group">
                <div className="flex items-center gap-3">
                  <Edit2 className="w-5 h-5 text-pink-300" />
        ...
        <DialogContent className="bg-[#1E003D] border-pink-400/30 text-white">
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Input type="password" placeholder="New password" className="bg-white/10 border-white/20 text-white" />
            </div>
            <div className="space-y-2">
              <Label className="text-white/70">Confirm Password</Label>
              <Input type="password" placeholder="Confirm password" className="bg-white/10 border-white/20 text-white" />
            </div>
            <Button className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-full" onClick={() => { setChangePasswordOpen(false); toast({ title: "🦋 Password updated!", description: "Your password has been changed." }); }}>
              Update Password
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={notifPrefsOpen} onOpenChange={setNotifPrefsOpen}>
        <DialogContent className="bg-[#1E003D] border-pink-400/30 text-white">
          <DialogHeader>
            ...
            <DialogDescription className="text-white/60">Choose what notifications you want to receive.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            {[
            ...
            <Button className="w-full bg-gradient-to-r from-pink-500 to-purple-600 text-white rounded-full" onClick={() => { setNotifPrefsOpen(false); toast({ title: "🦋 Preferences saved!", description: "Your notification settings have been updated." }); }}>
              Save Preferences
            </Button>
          </div>
        </DialogContent>
